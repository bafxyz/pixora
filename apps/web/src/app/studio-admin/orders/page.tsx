  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.error || _(msg`Failed to fetch orders`))
      }
    } catch (error) {
      console.error('Failed to fetch orders:', error)
      toast.error(_(msg`Failed to fetch orders`))
    } finally {
      setLoading(false)
    }
  }, [statusFilter, paymentFilter, _])
    ...
    <PageLayout title={_(msg`Orders Management`)}>
      ...
      <div className="space-y-4">
        {orders.length === 0 ? (
          ...
          orders.map((order) => (
            // biome-ignore lint/a11y/useSemanticElements: Large interactive card container with nested buttons
            <div
              key={order.id}
              role="button"
              tabIndex={0}
              className="bg-white border rounded-lg p-4 sm:p-6 hover:shadow-md transition-shadow cursor-pointer text-left w-full"
              onClick={() => router.push(`/studio-admin/orders/${order.id}`)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault()
                  router.push(`/studio-admin/orders/${order.id}`)
                }
              }}
            >
